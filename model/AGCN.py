...
class AVWGCN(nn.Module):
    ...
    def forward(self, x, adj_matrix):  # (64,170,65)
        ...
        x_g = torch.einsum("knm,bmc->bknc", supports, x)      # (64,2,170,65) B, cheb_k, N, dim_in
        x_g = x_g.permute(0, 2, 1, 3)  # (64,170,2,65) B, N, cheb_k, dim_in
        x_gconv = torch.einsum('bnki,kio->bno', x_g, self.weights) + self.bias   # self.weights:(2,128,128)    #b, N, dim_out
        return x_gconv
